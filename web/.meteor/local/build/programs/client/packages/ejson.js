...
EJSON.clone = function (v) {                                                                    // 351
  ...
  if (EJSON.isBinary(v)) {                                                                      // 359
    ret = EJSON.newBinary(v.length);                                                            // 360
    for (var i = 0; i < v.length; i++) {                                                        // 361
      ret[i] = v[i];                                                                            // 362
    }                                                                                           // 363
    return ret;                                                                                 // 364
  }                                                                                             // 365
  // XXX: Use something better than underscore's isArray                                        // 366
  if (_.isArray(v) || _.isArguments(v)) {                                                       // 367
    // For some reason, _.map doesn't work in this context on Opera (weird test                 // 368
    // failures).                                                                               // 369
    ret = [];                                                                                   // 370
    for (i = 0; i < v.length; i++)                                                              // 371
      ret[i] = EJSON.clone(v[i]);                                                               // 372
    return ret;                                                                                 // 373
  }                                                                                             // 374
  // handle general user-defined typed Objects if they have a clone method                      // 375
  if (typeof v.clone === 'function') {                                                          // 376
    return v.clone();                                                                           // 377
  }                                                                                             // 378
  // handle other custom types                                                                  // 379
  if (EJSON._isCustomType(v)) {                                                                 // 380
    return EJSON.fromJSONValue(EJSON.clone(EJSON.toJSONValue(v)), true);                        // 381
  }                                                                                             // 382
  // handle other objects                                                                       // 383
  ret = {};                                                                                     // 384
  _.each(v, function (value, key) {                                                             // 385
    ret[key] = EJSON.clone(value);                                                              // 386
  });                                                                                           // 387
  return ret;                                                                                   // 388
};                                                                                              // 389
                                                                                                // 390
...
//# sourceMappingURL=c09c5fa12d082a4f1ad121546574ef1d9a32180d.map
